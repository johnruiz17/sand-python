...
This program implements a two-dimensional sand world that the user can manipulate and simulates the effect of gravity
and Brownian motion.
...
def do_move(grid, x1, y1, x2, y2):
    ...
    """
    # moves value that is at x1, y1 to x2, y2. changes starting position to None
...
def check_move(grid, x1, y1, x2, y2):
    ...
    False
    """
    # these variables store the the maximum value for x and y
    max_x = len(grid[0]) - 1
    max_y = len(grid) - 1

    # returns False if move is invalid, returns True otherwise
    if x2 < 0 or x2 > max_x or y2 > max_y:
        return False
    elif grid[y2][x2] != None:
        return False
    elif grid[y1][x2] != None and x1 != x2:
        return False
    return True
...
def do_gravity(grid, x, y):
    ...
    # if sand, moves sand down if it is a valid move
    if grid[y][x] != 's':
        pass
...
def do_brownian(grid, x, y, brownian):
    ...
    # if sand, uses num and coin to determine if sand will be moved and if so to which left or right position
    if grid[y][x] != 's':
        pass
    elif num < brownian:
...
def do_whole_grid(grid, brownian):
    ...
    # calls do_gravity and do_brownian once for each (x, y) location in the grid
